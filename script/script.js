...
var authKey = "Basic VmNEOHVzWEVFRFdYNzhtTDB1UkY6WA=="
let callCount = 1;
let backlogCount = 0;
let readyCount = 0;
let inprogCount = 0;
...
function authorizeUser() {
  let pwUser = document.getElementById("pwUser").value;
  console.log(pwUser)
  if (pwUser == "transcom") {
    ...
    $("form")[0].reset();
    document.getElementById("wrongPassword").innerHTML = `Feil passord!`;
  }
}
...
//Agent id's //
      ...
      if (ticketStatus == "2" || ticketStatus == "9") {
        ...
        backlogCount += 1;
        var dropDown = `<div class="btn-group"><button type="button" class="btn btn-sm btn-dark dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></button><div class="dropdown-menu"><a class="dropdown-item" onclick='moveToReady("${ticketId}")'>Ready</a><a class="dropdown-item" onclick='moveToInProgress("${ticketId}")'>In progress</a><a class="dropdown-item" onclick='moveToBlocked("${ticketId}")'>Blocked</a><a class="dropdown-item" onclick='moveToDone("${ticketId}")'>Done</a>`
      
      } else if (ticketStatus == "11") {
        var taskType = "#readyboard";
        readyCount += 1;
        var dropDown = `<div class="btn-group"><button type="button" class="btn btn-sm btn-dark dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></button><div class="dropdown-menu"><a class="dropdown-item" onclick='moveToBacklog("${ticketId}")'>Backlog</a><a class="dropdown-item" onclick='moveToInProgress("${ticketId}")'>In progress</a><a class="dropdown-item" onclick='moveToBlocked("${ticketId}")'>Blocked</a><a class="dropdown-item" onclick='moveToDone("${ticketId}")'>Done</a>`

      } else if (ticketStatus == "10") {
        var taskType = "#inprogressboard";
        inprogCount += 1;
        var dropDown = `<div class="btn-group"><button type="button" class="btn btn-sm btn-dark dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></button><div class="dropdown-menu"><a class="dropdown-item" onclick='moveToBacklog("${ticketId}")'>Backlog</a><a class="dropdown-item" onclick='moveToReady("${ticketId}")'>Ready</a><a class="dropdown-item" onclick='moveToBlocked("${ticketId}")'>Blocked</a><a class="dropdown-item" onclick='moveToDone("${ticketId}")'>Done</a>`

      } else if (ticketStatus == "8" || ticketStatus == "7" || ticketStatus == "3") {
        var taskType = "#blockedboard";
        blockedCount += 1;
        var dropDown = `<div class="btn-group"><button type="button" class="btn btn-sm btn-dark dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></button><div class="dropdown-menu"><a class="dropdown-item" onclick='moveToBacklog("${ticketId}")'>Backlog</a><a class="dropdown-item" onclick='moveToReady("${ticketId}")'>Ready</a><a class="dropdown-item" onclick='moveToInProgress("${ticketId}")'>In progress</a><a class="dropdown-item" onclick='moveToDone("${ticketId}")'>Done</a>`

      } else if (ticketStatus == "4") {
        var taskType = "#doneboard";
        doneCount += 1;
        var dropDown = `<div class="btn-group"><div class="btn-group dropleft" role="group"><button type="button" class="btn btn-sm btn-dark dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></button><div class="dropdown-menu"><a class="dropdown-item" onclick='moveToBacklog("${ticketId}")'>Backlog</a><a class="dropdown-item" onclick='moveToReady("${ticketId}")'>Ready</a><a class="dropdown-item" onclick='moveToInProgress("${ticketId}")'>In progress</a><a class="dropdown-item" onclick='moveToBlocked("${ticketId}")'>Blocked</a></div>`

      } else {
        var taskType = "null";
      }

      $(taskType).append(`<div class='task'><div class='task__tags'>${ticketPriority} <button class='task__options'><i class='fas fa-ellipsis-h'></i></button>${dropDown}</div></div></div><div id="test" data-toggle='modal' data-target='#myModal'  onclick='previewTicket("${ticketId}")'><hr><p>${ticketSubject}</p><div class='task__stats'><span><time datetime='2021-11-24T20:00:00'><i class='fas fa-flag'></i>Opprettet: ${ticketCreated} || id: ${ticketId}</time><small style='position: absolute; bottom: 0; right: 0; width: 100px; text-align:right;'>${backlogState}</small> ${ticketOwner}</div>`)
...
function previewTicket(ticketId) {
  ...
  $("#commentsdesc, #modaldesc, #notefield").empty();
  $.ajax(settings).done(function (response) {
    console.log(response)
    ...
    var responderName = element["user_id"]
      ...
      }else if (responderName == "101058629252") {
        var commentOwner = "Via kanbanBoard"
      }else if (response["from_email"].includes('@')) {
...
function moveToBacklog(ticketId) {
  const statusId = 9
...
function moveToInProgress(ticketId) {
  const statusId = 10
  moveTicket(statusId, ticketId)
}
function moveToBlocked(ticketId) {
  const statusId = 8
  moveTicket(statusId, ticketId)
}
function moveToDone(ticketId) {
  const statusId = 4
  moveTicket(statusId, ticketId)
}

function moveTicket(statusId, ticketId) {
  var settings = {
